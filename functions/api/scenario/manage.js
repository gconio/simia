...
export async function onRequest({ request, env }) {
  ...
  try{
    if(method === "GET"){
      ...
      let current = null;
      if(targetId){
        current = await env.DB.prepare("SELECT id,name,subtitle,description,status,current_phase,start_at,created_at,updated_at FROM scenarios WHERE id=?1").bind(targetId).first();
      }
    ...
    if(method === "POST"){
      ...
      await env.DB.prepare(
        "INSERT INTO scenarios (id,name,subtitle,description,status,current_phase,start_at,created_at,updated_at) VALUES (?1,?2,?3,?4,?5,?6,?7,?8,?8) " +
        "ON CONFLICT(id) DO UPDATE SET name=excluded.name, subtitle=excluded.subtitle, description=excluded.description, status=excluded.status, current_phase=excluded.current_phase, start_at=excluded.start_at, updated_at=excluded.updated_at"
      ).bind(id,name,subtitle,description,status,current_phase,start_at,ts).run();

      if(body.make_current === true || body.make_current === 1 || body.make_current === "1" || !existing){
        await setCurrentScenarioId(env, id);
      }

      await env.DB.prepare("INSERT INTO scenario_actions (id,scenario_id,inject_id,action_type,actor,notes,ts) VALUES (?1,?2,NULL,?3,?4,?5,?6)")
        .bind(uuidv4(), id, existing ? "update" : "create", String(body.actor || "admin"), `Scenario ${existing ? "updated" : "created"}`, ts).run();
