export async function onRequest({ request, env }) {
  ...
  try {
    if (method === "GET") {
      const rows = await env.DB.prepare("SELECT id,label,icon,updated_at FROM teams ORDER BY id ASC").all();
    ...
    if (method === "POST") {
      ...
      for(const t of body.teams){
        ...
        stmts.push(env.DB.prepare("INSERT INTO teams (id,label,icon,updated_at) VALUES (?1,?2,?3,?4)")
