...
export async function onRequest({ request, env }) {
  ...
  try {
    if (method === "GET") {
      ...
      if (id) {
        const row = await env.DB.prepare(
          "SELECT id, name, email, role, team_id, created_at FROM participants WHERE id=?1"
        ).bind(id).first();
    ...
    if (method === "POST") {
      ...
      ).bind(id, name, email, role, team_id, created_at).run();

      return new Response(JSON.stringify({ ok:true, id, role, team_id }), {
        headers: { "Content-Type":"application/json" }
      });
    }
